...
| 2026-09-20 | Anthropic Claude Code — Claude Opus 5 (`claude-opus-5`) | `Verify the ambient-light guidance and magnetometer heading, including missing-sensor fallbacks.` | `CompassHeading.kt`, magnetometer accuracy handling and capture-time heading in `SensorMonitor.kt`/`FloraGuideViewModel.kt`, `LightCondition` and pill/hint changes; `CompassHeadingTest.kt`, `SensorSnapshotTest.kt`; `docs/HARDWARE_ADAPTERS_VERIFICATION.md`. | Pose-based and threshold unit tests, lint and debug build pass. Compass bearings, lux thresholds and missing-sensor behaviour still to be checked on phones by Mason against the new checklist. | Mason |
